...
const NoundTest = (nightmare, delay) => {
    ...
    return nightmare
        ...
        .evaluate(function () {
            return document.querySelector('#nound-addedit-form') === null
        })

        .then(
            nound_add_form_gone => {
                if (!nound_add_form_gone)
                    throw('nound-addedit-form has not gone away after cancel')
            })
        .then( res => {
            return nightmare
                .click('#add-nound').wait(delay)
        ...
        .then(nound_add_form_gone => {
            if (!nound_add_form_gone)
                throw('nound-addedit-form has not gone away after save')
        })
        .then( res => {
            return nightmare
                .evaluate(function() {
